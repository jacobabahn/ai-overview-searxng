...
For a host installation, use the reachable host address described above.
...
Ruff lint/format checks, ty, 85 Python tests, 8 JavaScript lifecycle tests,
...
A live Ollama 0.34.2 test verified `qwen3.5:4b` with thinking disabled on a
Ryzen 7 PRO 6850U and integrated Radeon GPU using Vulkan. Ollama reported full
GPU offload at an 8,192-token context. The browser completed a real search
overview with citations in about 18 seconds and a follow-up using existing
evidence in about 23 seconds, with successful stream completion and no browser
errors. These are single-run observations, not a general performance benchmark.
